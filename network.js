...
function handleJustsaying(ws, subject, body){
	switch (subject){
		...
		case 'upgrade_required':
			...
			throw Error("Mandatory upgrade required, please check the release notes and upgrade.");
			break;
...
function handleRequest(ws, tag, command, params){
	...
	switch (command){
		...
		case 'get_peers':
			var arrPeerUrls = arrOutboundPeers.filter(function(ws){ return (ws.host !== 'uri' && ws.readyState === ws.OPEN && ws.bSubscribed && ws.bSource); }).map(function(ws){ return ws.peer; });
